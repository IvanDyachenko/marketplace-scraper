...
@derive(loggable)
final case class OutOfStockAnalogs(
  id: Item.Id,
  index: Int,
  `type`: Item.Type,
  title: String,
  ...
  val availability: Item.Availability = Item.Availability.OutOfStockAnalogs
}

object OutOfStockAnalogs {
  implicit val circeDecoder: Decoder[OutOfStockAnalogs] = Decoder.instance[OutOfStockAnalogs] { (c: HCursor) =>
    ...
    for {
      _    <- i.get[Item.Availability]("availability")
                .ensure {
                  val message =
                    s"'cellTrackingInfo' doesn't contain 'availability' which is equal to '${Item.Availability.OutOfStockAnalogs.value}'"
                  DecodingFailure(message, c.history)
                }(_ == Item.Availability.OutOfStockAnalogs)
      item <- (
                i.get[Item.Id]("id"),
                i.get[Int]("index"),
