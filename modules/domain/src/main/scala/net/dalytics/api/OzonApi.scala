...
trait OzonApi[F[_], S[_]] {
  ...
  def page(id: Category.Id, searchFilter: Option[SearchFilter] = None): F[Option[Page]]
  def soldOutPage(id: Category.Id, searchFilter: Option[SearchFilter] = None): F[Option[Page]]
  def searchFilters(id: Category.Id, searchFilterKey: SearchFilter.Key): F[Option[SearchFilters]]
...
object OzonApi {
  ...
  private final class Impl[F[_]: Concurrent: Logging: HttpClient: HttpClient.Handling] extends OzonApi[F, Stream[F, *]] {
    ...
    def page(id: Category.Id, searchFilter: Option[SearchFilter]): F[Option[Page]] = {
      val request = Request.GetCategorySearchResultsV2(id, 1 @@ Url.Page, searchFilter)
      get[Result](request).map(_ >>= (_.page))
    }

    def soldOutPage(id: Category.Id, searchFilter: Option[SearchFilter]): F[Option[Page]] = {
    ...
    new BifunctorK[OzonApi] {
      def bimapK[F[_]: Functor, G[_]: Functor, W[_], Q[_]](ufg: OzonApi[F, G])(fw: F ~> W)(gq: G ~> Q): OzonApi[W, Q] =
        new OzonApi[W, Q] {
          ...
          def searchFilters(id: Category.Id, key: SearchFilter.Key): W[Option[SearchFilters]]                              = fw(ufg.searchFilters(id, key))
