...
final case class PreOrder(
  ...
  val availability: Item.Availability = Item.Availability.PreOrder
}
  ...
  implicit val circeDecoder: Decoder[PreOrder] = Decoder.instance[PreOrder] { (c: HCursor) =>
    ...
    for {
      _    <- i.get[Item.Availability]("availability")
                .ensure {
                  val message = s"'cellTrackingInfo' doesn't contain 'availability' which is equal to '${Item.Availability.PreOrder.value}'"
                  DecodingFailure(message, c.history)
                }(_ == Item.Availability.PreOrder)
      item <- (
                i.get[Item.Id]("id"),
                i.get[Int]("index"),
