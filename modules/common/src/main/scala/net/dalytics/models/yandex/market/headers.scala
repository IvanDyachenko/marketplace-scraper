...
import org.typelevel.ci.{ CIString, _ }
  ...
  object `X-Region-Id` extends HeaderKey.Singleton {
    ...
    def name: CIString = ci"X-Region-Id"
    ...
    def matchHeader(header: Header.Raw): Option[`X-Region-Id`] =
      if (header.name == name) Region.GeoId(header.value).map(`X-Region-Id`(_))
      else None
  }

  final case class `X-Device-Type`(deviceType: String) extends Header.Raw.Parsed {
  ...
  object `X-Device-Type` extends HeaderKey.Singleton {
    ...
    def name: CIString = ci"X-Device-Type"

    def parse(s: String): ParseResult[`X-Device-Type`] = Right(`X-Device-Type`(s))

    def matchHeader(header: Header.Raw): Option[`X-Device-Type`] =
      if (header.name == name) Some(`X-Device-Type`(header.value))
      else None
  }

  final case class `X-Platform`(platform: String) extends Header.Raw.Parsed {
    def key                                      = `X-Platform`
  ...
  object `X-Platform` extends HeaderKey.Singleton {
    ...
    def name: CIString = ci"X-Platform"

    def parse(s: String): ParseResult[`X-Platform`] = Right(`X-Platform`(s))

    def matchHeader(header: Header.Raw): Option[`X-Platform`] =
      if (header.name == name) Some(`X-Platform`(header.value))
      else None
  }

  final case class `X-App-Version`(appVersion: String) extends Header.Raw.Parsed {
  ...
  object `X-App-Version` extends HeaderKey.Singleton {
    ...
    def name: CIString = ci"X-App-Version"

    def parse(s: String): ParseResult[`X-App-Version`] = Right(`X-App-Version`(s))

    def matchHeader(header: Header.Raw): Option[`X-App-Version`] =
