...
import fs2.kafka.{AutoOffsetReset, ConsumerSettings, KafkaConsumer, KafkaProducer, ProducerSettings, RecordDeserializer, RecordSerializer}
...
object KafkaClient {
  ...
  )(schemaRegistryClient: SchemaRegistryClient): Resource[F, KafkaProducer[F, K, V]] = {
  ...
  def makeConsumer[F[_]: ContextShift: ConcurrentEffect: Timer, K: VulcanCodec, V: VulcanCodec](
  ...
  )(schemaRegistryClient: SchemaRegistryClient): Resource[F, KafkaConsumer[F, K, V]] = {
    val avroSettings: AvroSettings[F] = makeAvroSettings(schemaRegistryClient)
    ...
    val consumerSettingsBase =
      ConsumerSettings[F, K, V](keyDeserializer, valueDeserializer)
        .withBootstrapServers(kafkaConfig.bootstrapServers)
    ...
    KafkaConsumer.resource[F, K, V](consumerSettingsWithEnableAutoCommit).evalTap(_.subscribeTo(kafkaConsumerConfig.topic))
  }
